...
def main():
    ...
    accelerator = Accelerator(
        log_with=args["report_to"], logging_dir=args["output_dir"], fp16=True,
    )
    ...
    for fold in range(2, cfg["k_folds"]):
        ...
        for epoch in range(args.num_train_epochs):
            ...
            for step, batch in enumerate(train_dataloader):
                ...
                temp_logits = outputs.logits.detach().cpu().numpy().astype(np.float16).ravel().tolist()
                temp_labels = batch["labels"].detach().cpu().numpy().astype(np.float16).ravel().tolist()
            ...
            with torch.no_grad():
                for step, batch in enumerate(eval_dataloader):
                    ...
                    temp[:, :seq_len] = outputs.logits.detach().cpu().numpy().squeeze()
                    predictions.append(temp.astype(np.float16))
